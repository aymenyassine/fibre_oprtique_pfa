...
public class RequestService {
    ...
    public RequestService(
        DemandeRaccordementRepository demandeRepository,
    ...
    public Page<DemandeRaccordementDto> search(
        DemandeStatus statut,
        String keyword,
        Pageable pageable
    ) {
        return demandeRepository
            .search(statut, keyword, pageable)
            .map(DemandeRaccordementDto::fromEntity);
    }

    /** Technicien view — only demandes assigned to him. */
    public Page<DemandeRaccordementDto> searchForTechnicien(
        Long technicienId,
        DemandeStatus statut,
        String keyword,
        Pageable pageable
        ...
        return demandeRepository
            .searchByTechnicien(technicienId, statut, keyword, pageable)
            .map(DemandeRaccordementDto::fromEntity);
    }
     ...
     */
    @Transactional
    public DemandeRaccordementDto submitRequest(
        DemandeRaccordementRequest request
    ) {
        ...
        if (request.getLatitude() != null && request.getLongitude() != null) {
            ...
            eligibility = EligibilityResponse.builder()
                .eligible(false)
                .message(
                    "Coordonnées GPS non fournies — vérification manuelle requise."
                )
                .build();
        ...
        DemandeRaccordement demande = DemandeRaccordement.builder()
            ...
            .offreIdChoisie(request.getIfOffer())
    ...
    public DemandeRaccordementDto setDevis(Long id, DevisRequest request) {
        ...
        if (
            demande.getStatut() != DemandeStatus.EN_ANALYSE &&
            demande.getStatut() != DemandeStatus.SOUMISE
        ) {
            throw new DemandeValidationException(
                "Le devis ne peut être traité que pour une demande SOUMISE ou EN_ANALYSE." +
                    " Statut actuel : " +
                    demande.getStatut()
            );
        }
        ...
        if (
            request.getDecision() ==
            com.fibre.optique.request.dto.DevisDecision.ANNULE
        ) {
            // Validation : raison obligatoire en cas d'annulation
            if (request.getRaisonAnnulation() == null) {
                throw new DemandeValidationException(
            ...
            if (
                ...
                request.getCommentaire().isBlank()
            ) {
                throw new DemandeValidationException(
                    "Un commentaire explicatif est obligatoire en cas d'annulation."
                );
            }
            demande.setStatut(DemandeStatus.REJETEE);
            demande.setRaisonAnnulation(request.getRaisonAnnulation().name());
            demande.setCommentaireTraitement(request.getCommentaire());
        } else {
            // CONFIRME
            if (demande.getOffreIdChoisie() == null) {
                throw new DemandeValidationException(
                    "Aucune offre n'a été sélectionnée par le prospect."
                );
            }
            com.fibre.optique.offer.entity.Offre offer =
                offerService.getOfferEntityById(demande.getOffreIdChoisie());
            demande.setMontantDevis(offer.getPrixHT());
            demande.setCommentaireTraitement(request.getCommentaire());
            demande.setStatut(DemandeStatus.DEVIS_ENVOYE);
        }

        return DemandeRaccordementDto.fromEntity(
    ...
    public DemandeRaccordementDto schedule(Long id, ScheduleRequest request) {
        ...
        requireStatus(
            demande,
            DemandeStatus.ACCEPTEE,
            "Une demande doit être acceptée avant d'être planifiée."
        );

        User technicien = userService
            .findById(request.getTechnicienId())
            .orElseThrow(() ->
                new DemandeValidationException(
                    "Technicien introuvable avec l'identifiant : " +
        ...
        if (technicien.getRole() != Role.TECHNICIEN) {
            throw new DemandeValidationException(
                "L'utilisateur désigné n'a pas le rôle TECHNICIEN : " +
                    technicien.getEmail()
            );
        }
    ...
    public DemandeRaccordementDto complete(Long id) {
        ...
        if (user.getRole() == Role.PROSPECT) {
            ...
            log.info(
                "🔄 Rôle changé: PROSPECT → CLIENT pour {} (ID: {})",
                user.getEmail(),
                user.getId()
            );
        }
        ...
        return userService
            ...
            .map(existingUser -> {
                ...
                if (existingUser.getRole() == Role.PROSPECT) {
                    log.info(
                        "👤 Utilisateur PROSPECT existant, réutilisation ID: {}",
                        existingUser.getId()
                    );
                    return existingUser;
                }
                ...
                log.info(
                    "👤 Utilisateur avec rôle {} trouvé, réutilisation",
    ...
    public DemandeRaccordementDto reject(Long id, String raison) {
        ...
        if (demande.getStatut() == DemandeStatus.TERMINE) {
            throw new DemandeValidationException(
                "Une demande terminée ne peut pas être rejetée."
            );
        }
    ...
    private void requireStatus(
        DemandeRaccordement demande,
        DemandeStatus required,
        String message
    ) {
    ...
    private UserDto provisionClientAccount(DemandeRaccordement demande) {
        ...
        return userService
            ...
            .map(existingUser -> {
                log.info(
                    "User account already exists for {}, reusing id={}",
                    demande.getProspectEmail(),
                    existingUser.getId()
                );
                return com.fibre.optique.users.dto.UserDto.fromEntity(
                    existingUser
                );
            })
            .orElseGet(() -> {
